The player characters can't see the full game world, they can only see a small section of it.

Weapons, armour, and items all have a unique ID which refers to a specific weapon/armor/item entity. 
...
The key to updating the grid of tiles that are around the player is the `worldState.frames` section of the JsonFrameUpdate JSON. 

The frames field communicates all of the tile changes that occurred in the world since the previous frame. In the case of a full update, the frames field will contain all 80x40 tiles. However, for non-full frames, it will contain only the tiles that have been updated. 
...
For example, if x = 2, y = 2, w = 2, and h = 2, the array would represent coordinate data in this order: 
```
[ 
  [data for (2, 2) coord], 
  [data for (2, 3) coord], 
...
```
Thus the array is a single dimensional array, that maps onto a 2D rectangle, from coordinate (2,2) at the top left, to coordinate (3,3) at the bottom right.
...
- One or more "layers", from top to bottom, which indicate what graphics to display on the tile

All graphics in the game come from the RogueCloudResources project, under `com.roguecloud.resources.tiles`. They are numbered PNG files, with names like `202.png`. Each layer contains a number (indicating which PNG file to display at that coordinate).
...
Thus, putting it all together, a layer looks like this:
```
[
	(1 or 0 = 1 if the player can enter it, 0 otherwise), 
	layer 1,
	layer 2,
	(...),
	layer n
...
So here's a fully JSON representation of a tile square data:
```
...
In this example, the server is telling us that a tile at the position of (40, 20) has changed.
...
Here is an example of one of the creature object inside the `visibleObjects` field:
```
  {
    "position": {
      "x": 122,
...
You notice that the weapon ID is a number, rather than a JSON object describing the weapon. The weapon object can be found in the `weapons` JSON field, which will include a weapon with id 20.
...
Note that the above object it does not have a world coordinate. This is because the world position of an object is contained in the `worldState.visibleObjects` field, like so:
```
      {
        "position": {
          "x": 326,
...
```
The containedObjectId refers to the weapon ID 22, while the objectId 49 is the id for a special type of object called a visible object (see `JsonVisibleObject`). A visible object is just an object with a position in the world. The reason that the weapon and visible object are kept separate is per the DRY principle mentioned above. This allows us to reduce server -> client bandwidth.
